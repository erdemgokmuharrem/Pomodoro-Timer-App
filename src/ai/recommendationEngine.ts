...
export class RecommendationEngine {
  ...
  private generateTaskRecommendations(context: ContextualData): Recommendation[] {
    const recommendations: Recommendation[] = [];
    const currentHour = context.currentTime.getHours();
    
    // Energy-based task recommendations
    if (this.userBehavior.workingHours.includes(currentHour)) {
      const highEnergyTasks = this.tasks.filter(task => 
        !task.isCompleted && 
        task.priority === 'high' && 
      ...
      if (highEnergyTasks.length > 0) {
        recommendations.push({
          ...
          description: 'Şu anda enerji seviyeniz yüksek. Zorlu görevlere odaklanın.',
          confidence: 0.8,
          priority: 'high',
    ...
    if (context.upcomingDeadlines > 0) {
      ...
      if (urgentTasks.length > 0) {
        recommendations.push({
          ...
          type: 'task',
          title: 'Acil Görev',
          description: 'Yaklaşan deadline\'lar var. Acil görevlere öncelik verin.',
          confidence: 0.9,
          priority: 'high',
    ...
    if (this.userBehavior.focusScore < 70) {
      recommendations.push({
        ...
        description: 'Odaklanma skorunuz düşük. Kısa, basit görevlerle başlayın.',
        confidence: 0.7,
        priority: 'medium',
  ...
  private generateBreakRecommendations(context: ContextualData): Recommendation[] {
    const recommendations: Recommendation[] = [];
  ...
  private generateScheduleRecommendations(context: ContextualData): Recommendation[] {
    const recommendations: Recommendation[] = [];
    ...
    if (this.userBehavior.productivityTrend === 'decreasing') {
      recommendations.push({
        ...
        description: 'Son dönemde verimliliğiniz düşüyor. Çalışma rutininizi gözden geçirin.',
        confidence: 0.8,
        priority: 'high',
  ...
  private generateHabitRecommendations(context: ContextualData): Recommendation[] {
    const recommendations: Recommendation[] = [];
    ...
    if (consistencyScore < 0.6) {
      recommendations.push({
        ...
        description: 'Çalışma tutarlılığınızı artırın. Her gün aynı saatte başlayın.',
        confidence: 0.7,
        priority: 'medium',
  ...
  private calculateConsistencyScore(): number {
    ...
    const recentSessions = this.historicalData.filter(session => 
      new Date(session.startTime) > lastWeek
    );
