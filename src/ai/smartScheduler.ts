...
export class SmartScheduler {
  ...
  scheduleBreak(currentSession: PomodoroSession): Date {
    ...
    const adjustedDuration = this.adjustBreakDuration(
      baseBreakDuration,
      energyLevel,
      sessionDuration
    );
  ...
  private getPreferredBreakTimes(): number[] {
    ...
    const average =
      breakDurations.reduce((sum, duration) => sum + duration, 0) /
      breakDurations.length;
    return [Math.round(average), Math.round(average * 2)];
  }

  private getAverageSessionLength(): number {
    const completedSessions = this.historicalData.filter(
      session => session.isCompleted && !session.isBreak
    );
  ...
  private calculateCurrentEnergyLevel(hour: number): number {
    const isProductiveHour =
      this.userPatterns.mostProductiveHours.includes(hour);
  ...
  private predictEnergyLevel(currentTime: Date): number {
    const currentEnergy = this.calculateCurrentEnergyLevel(
  ...
  private analyzeEnergyTrend(): number {
    ...
    const earlySessions = recentSessions.slice(
      0,
      Math.floor(recentSessions.length / 2)
    );
    const lateSessions = recentSessions.slice(
      Math.floor(recentSessions.length / 2)
    );

    const earlyAvgInterruptions =
      earlySessions.reduce((sum, s) => sum + s.interruptions, 0) /
      earlySessions.length;
    const lateAvgInterruptions =
      lateSessions.reduce((sum, s) => sum + s.interruptions, 0) /
      lateSessions.length;
