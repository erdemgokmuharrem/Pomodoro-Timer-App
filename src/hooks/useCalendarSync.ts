...
export const useCalendarSync = (): UseCalendarSyncReturn => {
  ...
  const updateSettings = useCallback(
    async (newSettings: Partial<CalendarSyncSettings>) => {
      try {
  ...
  const refreshEvents = useCallback(
    async (startDate: Date, endDate: Date) => {
      ...
      try {
        const calendarEvents = await calendarService.getAllEvents(
          startDate,
          endDate
        );
  ...
  const createPomodoroEvent = useCallback(
    async (
      taskTitle: string,
      startTime: Date,
      duration: number,
      ...
      try {
        return await calendarService.createPomodoroEvent(
          taskTitle,
          startTime,
          duration,
          isBreak
  ...
  const createTaskEvent = useCallback(
    async (
      ...
      try {
        return await calendarService.createTaskEvent(
          taskTitle,
          startTime,
          endTime,
          description
  ...
  const checkTimeConflict = useCallback(
    async (startTime: Date, endTime: Date): Promise<CalendarEvent[]> => {
      try {
