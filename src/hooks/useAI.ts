...
import { SmartScheduler, SmartSuggestion, EnergyAnalysis } from '../ai/smartScheduler';
import { RecommendationEngine, Recommendation, ContextualData } from '../ai/recommendationEngine';
...
export const useAI = () => {
  ...
  const smartScheduler = new SmartScheduler(
    {
      ...
      focusScore: userStats.totalFocusTime > 0 ? Math.min(100, (userStats.totalFocusTime / 1000) * 10) : 50,
      energyLevel: userStats.currentStreak > 7 ? 'high' : userStats.currentStreak > 3 ? 'medium' : 'low',
      mood: userStats.currentStreak > 5 ? 'positive' : userStats.currentStreak > 2 ? 'neutral' : 'negative',
    },
    sessions,
    tasks
  );
  ...
  const recommendationEngine = new RecommendationEngine(
    {
      ...
      taskCompletionRate: tasks.length > 0 ? tasks.filter(t => t.isCompleted).length / tasks.length : 0,
      focusScore: userStats.totalFocusTime > 0 ? Math.min(100, (userStats.totalFocusTime / 1000) * 10) : 50,
      productivityTrend: userStats.currentStreak > 5 ? 'increasing' : userStats.currentStreak > 2 ? 'stable' : 'decreasing',
  ...
  const generateRecommendations = useCallback(async (context?: Partial<ContextualData>) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      const contextualData: ContextualData = {
        ...
        upcomingDeadlines: tasks.filter(t => !t.isCompleted && t.priority === 'high').length,
        recentInterruptions: sessions
          .filter(s => new Date(s.startTime) > new Date(Date.now() - 24 * 60 * 60 * 1000))
          .reduce((sum, s) => sum + s.interruptions, 0),
        ...context,
      };
      
      const newRecommendations = recommendationEngine.generateRecommendations(contextualData);
  ...
  const getOptimalWorkTimes = useCallback((days: number = 7) => {
    return smartScheduler.predictOptimalWorkTimes(days);
  }, [smartScheduler]);

  // Schedule smart break
  const scheduleSmartBreak = useCallback((currentSession: any) => {
  ...
  const getTaskSuggestion = useCallback((taskId: string) => {
  ...
  }, [tasks.length, sessions.length, generateSuggestions, generateRecommendations, analyzeEnergy]);
