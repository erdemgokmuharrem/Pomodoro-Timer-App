...
export interface UseAppBlockingReturn {
  ...
  updateBlockedWebsite: (websiteId: string, updates: Partial<BlockedWebsite>) => Promise<void>;
...
export const useAppBlocking = (): UseAppBlockingReturn => {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        const currentInstalledApps = await appBlockingService.getInstalledApps();
  ...
  // Update settings
  const updateSettings = useCallback(async (newSettings: Partial<BlockingSettings>) => {
    try {
  ...
  const shouldBlockApp = useCallback((packageName: string, isPomodoroActive: boolean, isBreakActive: boolean) => {
    return appBlockingService.shouldBlockApp(packageName, isPomodoroActive, isBreakActive);
  }, []);

  const shouldBlockWebsite = useCallback((url: string, isPomodoroActive: boolean, isBreakActive: boolean) => {
    return appBlockingService.shouldBlockWebsite(url, isPomodoroActive, isBreakActive);
  }, []);
  ...
  const unblockApp = useCallback(async (packageName: string): Promise<boolean> => {
    try {
  ...
  const recordBlock = useCallback(async (appName: string, websiteName?: string) => {
    try {
