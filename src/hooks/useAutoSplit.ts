...
export const useAutoSplit = () => {
  ...
  const [splitRules, setSplitRules] = useState<SplitRule[]>([
    ...
    // Rule 3: High priority long tasks
    {
      ...
      condition: (task) => task.priority === 'high' && task.estimatedPomodoros >= 4,
      splitStrategy: (task) => {
        const tasks: Task[] = [];
    ...
    // Rule 4: Learning/Study tasks
    {
      ...
      condition: (task) => task.tags.some(tag => ['öğrenme', 'eğitim', 'çalışma', 'okuma'].includes(tag.toLowerCase())),
      splitStrategy: (task) => {
        const tasks: Task[] = [];
  ...
  const analyzeTask = useCallback((task: Task): SplitResult | null => {
    ...
    const totalPomodoros = splitTasks.reduce((sum, t) => sum + t.estimatedPomodoros, 0);
  ...
  // Auto-split a task
  const autoSplitTask = useCallback((task: Task): SplitResult | null => {
  ...
  const getSplitSuggestions = useCallback((task: Task) => {
    const splitResult = analyzeTask(task);
    if (!splitResult) return null;
  ...
  const bulkSplitTasks = useCallback((taskIds: string[]) => {
  ...
  // Update split rules
  const updateSplitRule = useCallback((ruleId: string, updates: Partial<SplitRule>) => {
