...
export const useWeeklyReview = () => {
  ...
  const getWeekSessions = useCallback((startDate: Date, endDate: Date) => {
    if (!sessions) return [];
    
    return sessions.filter(session => {
      const sessionDate = new Date(session.startTime);
      return sessionDate >= startDate && sessionDate <= endDate && session.isCompleted;
    });
  }, [sessions]);

  // Get tasks for a specific week
  const getWeekTasks = useCallback((startDate: Date, endDate: Date) => {
    if (!tasks) return [];
  ...
  const calculateWeeklyStats = useCallback((weekSessions: PomodoroSession[], weekTasks: Task[]): WeeklyStats => {
    const totalPomodoros = weekSessions.length;
    const completedTasks = weekTasks.filter(task => task.isCompleted).length;
    const totalTasks = weekTasks.length;
    
    const averageSessionDuration = weekSessions.length > 0 
      ? weekSessions.reduce((sum, session) => sum + session.duration, 0) / weekSessions.length 
      : 0;
    
    const totalFocusTime = weekSessions.reduce((sum, session) => sum + session.duration, 0);
    const interruptions = weekSessions.reduce((sum, session) => sum + session.interruptions, 0);
    
    // Calculate productivity score (0-100)
    const productivityScore = Math.min(100, (totalPomodoros * 10) + (completedTasks * 5));
    ...
    const energyLevel = morningSessions > totalPomodoros * 0.4 ? 'high' : 
                       morningSessions > totalPomodoros * 0.2 ? 'medium' : 'low';
    
    // Calculate focus score (0-100)
    const focusScore = interruptions > 0 ? Math.max(0, 100 - (interruptions * 10)) : 100;
    
    // Calculate consistency score (0-100)
    const daysWithSessions = new Set(weekSessions.map(session => 
  ...
  const generateInsights = useCallback((stats: WeeklyStats, weekSessions: PomodoroSession[], weekTasks: Task[]): WeeklyInsights => {
    ...
    if (stats.totalPomodoros >= 20) {
      insights.achievements.push('Harika! Bu hafta 20+ pomodoro tamamladınız 🎉');
    }
    ...
    if (stats.interruptions > stats.totalPomodoros * 0.3) {
      insights.challenges.push('Kesinti sayısı yüksek - dikkat dağıtıcıları azaltmayı düşünün');
    }
    if (stats.consistencyScore < 50) {
      insights.challenges.push('Tutarlılık düşük - düzenli çalışma rutini oluşturun');
    }
    if (stats.focusScore < 70) {
      insights.challenges.push('Odaklanma sorunu var - sessiz ortam arayın');
    }
    if (stats.energyLevel === 'low') {
      insights.challenges.push('Enerji seviyesi düşük - uyku ve beslenme düzeninizi kontrol edin');
    }
    ...
    if (stats.interruptions > 5) {
      insights.recommendations.push('Kesintileri azaltmak için bildirimleri kapatın');
    }
    ...
    if (stats.energyLevel === 'high') {
      insights.energyTrends.push('Enerji seviyeniz yüksek, bu momentumu koruyun!');
    } else if (stats.energyLevel === 'low') {
      insights.energyTrends.push('Enerji seviyeniz düşük, dinlenme ve beslenme düzeninizi gözden geçirin');
    }
    ...
    if (stats.productivityScore >= 80) {
      ...
      insights.productivityTrends.push('Üretkenlik seviyenizi artırmak için daha fazla odaklanma gerekli');
    }
  ...
  const generateWeeklyReview = useCallback((): WeeklyReview => {
    ...
    return review;
  }, [getCurrentWeekRange, getWeekSessions, getWeekTasks, calculateWeeklyStats, generateInsights]);
  ...
  const getWeeklyReviewHistory = useCallback(() => {
    return weeklyReviews.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
  }, [weeklyReviews]);
  ...
  const getWeeklyProgressComparison = useCallback(() => {
    ...
    const pomodoroChange = currentWeek.stats.totalPomodoros - previousWeek.stats.totalPomodoros;
    const taskChange = currentWeek.stats.completedTasks - previousWeek.stats.completedTasks;
    const productivityChange = currentWeek.stats.productivityScore - previousWeek.stats.productivityScore;
