...
export interface FocusRoom {
  ...
  theme: 'forest' | 'ocean' | 'mountain' | 'city' | 'space' | 'library' | 'cafe';
...
interface FocusRoomsStore {
  ...
  // Actions
  createRoom: (roomData: Omit<FocusRoom, 'id' | 'hostId' | 'members' | 'chat' | 'statistics'>) => Promise<string>;
  joinRoom: (roomId: string, memberData: Omit<FocusRoomMember, 'id' | 'joinTime' | 'focusStreak' | 'totalSessions' | 'isHost' | 'isModerator'>) => Promise<boolean>;
  ...
  updateRoomSettings: (roomId: string, settings: Partial<FocusRoom['settings']>) => Promise<boolean>;
  ...
  updateRoomRules: (roomId: string, rules: Partial<FocusRoom['rules']>) => Promise<boolean>;
  
  // Chat & Interactions
  sendMessage: (roomId: string, message: string, type?: 'text' | 'emoji' | 'encouragement' | 'achievement') => Promise<boolean>;
  sendEncouragement: (roomId: string, encouragement: string, targetUserId?: string) => Promise<boolean>;
  shareAchievement: (roomId: string, achievement: string) => Promise<boolean>;
  
  // Room discovery
  searchRooms: (query: string, category?: string, theme?: string) => Promise<FocusRoom[]>;
  getRecommendedRooms: () => Promise<FocusRoom[]>;
  getTrendingRooms: () => Promise<FocusRoom[]>;
  
  // Statistics
  updateRoomStatistics: (roomId: string, memberId: string, focusCompleted: boolean) => Promise<void>;
...
export const useFocusRoomsStore = create<FocusRoomsStore>((set, get) => ({
          ...
          room.id === roomId 
            ? { 
                ...
                statistics: {
                  ...
                  activeMembers: room.members.filter(m => m.isOnline && m.id !== memberId).length,
                },
              }
            : room
        );
  ...
  startRoom: async (roomId) => {
    try {
      ...
      set({
        ...
        currentRoom: updateRoom([...availableRooms, ...myRooms, ...joinedRooms].find(r => r.id === roomId) || null),
      });
  ...
  sendEncouragement: async (roomId, encouragement, targetUserId) => {
    try {
      ...
      const encouragementMessage = {
        ...
        userId: 'current-user-id',
        userName: 'You',
        message: targetUserId ? `@${targetUserId} ${encouragement}` : encouragement,
        timestamp: new Date(),
      ...
      const updateRoom = (rooms: FocusRoom[]) =>
        rooms.map(room => {
          if (room.id === roomId) {
            const updatedMessages = [...room.chat.messages, encouragementMessage];
            return {
              ...room,
        ...
        rooms.map(room => {
          if (room.id === roomId) {
            const updatedMembers = room.members.map(member =>
              member.id === memberId
                ? { ...member, focusStreak: member.focusStreak + (focusCompleted ? 1 : 0) }
                : member
            );

            const totalSessions = room.statistics.totalSessions + 1;
            const totalFocusTime = room.statistics.totalFocusTime + (focusCompleted ? room.settings.pomodoroDuration : 0);
  ...
  updateSettings: async (newSettings) => {
    try {
      ...
      await AsyncStorage.setItem('focusRoomsSettings', JSON.stringify(updatedSettings));
    } catch (error) {
  ...
  loadRooms: async () => {
    try {
      ...
      if (savedRooms) {
        ...
        set({ 
          availableRooms: rooms.filter((r: FocusRoom) => r.isPublic && !r.isActive),
          myRooms: rooms.filter((r: FocusRoom) => r.hostId === 'current-user-id'),
          joinedRooms: rooms.filter((r: FocusRoom) => r.members.some(m => m.id === 'current-user-id')),
        });
      }
