...
interface SocialChallengesStore {
  ...
  createChallenge: (
    challengeData: Omit<SocialChallenge, 'id' | 'participants' | 'statistics'>
  ) => Promise<string>;
  joinChallenge: (
    challengeId: string,
    participantData: Omit<
      ChallengeParticipant,
      'id' | 'joinTime' | 'lastActivity' | 'achievements' | 'rank'
    >
  ) => Promise<boolean>;
  leaveChallenge: (
    challengeId: string,
  ...
  // Team management
  createTeam: (
    teamData: Omit<ChallengeTeam, 'id' | 'createdDate' | 'isActive'>
  ) => Promise<string>;
...
export const useSocialChallengesStore = create<SocialChallengesStore>(
  (set, get) => ({
    // Initial state
    activeChallenges: [],
    myChallenges: [],
        ...
        const updateChallenge = (challenges: SocialChallenge[]) =>
          challenges.map(challenge => {
            if (challenge.id === challengeId) {
              const updatedParticipants = challenge.participants.map(
                participant =>
    ...
    joinTeam: async (teamId, participantId) => {
      try {
        ...
        const updatedTeam = {
          ...
          members: [
            ...team.members,
            { id: participantId } as ChallengeParticipant,
          ],
        };
    ...
    getTrendingChallenges: async () => {
      try {
        ...
        return activeChallenges
          ...
          .sort(
            (a, b) =>
              b.statistics.totalParticipants - a.statistics.totalParticipants
          )
          .slice(0, 10);
      } catch (error) {
    ...
    getTeamLeaderboard: async challengeId => {
      try {
        const { myTeams } = get();
        const challengeTeams = myTeams.filter(
          team => team.challengeId === challengeId
        );
    ...
    claimReward: async (challengeId, rewardType) => {
      try {
        ...
        console.log(
          `Claiming ${rewardType} reward for challenge ${challengeId}`
        );
        return true;
      } catch (error) {
    ...
    completeChallenge: async (challengeId, participantId) => {
      try {
        ...
        set({
          completedChallenges: [...completedChallenges, updatedChallenge],
        });
    ...
    getCompletionStats: async challengeId => {
      try {
        ...
        const completedParticipants = challenge.participants.filter(
          p => p.currentProgress >= challenge.targetValue
        ).length;
        const averageProgress =
