...
import { useComplexityScore, ComplexityScore } from '../../hooks/useComplexityScore';
...
export const ComplexityScoreModal: React.FC<ComplexityScoreModalProps> = ({
  ...
  const [selectedTaskComplexity, setSelectedTaskComplexity] = useState<ComplexityScore | null>(null);
  ...
  }, [visible, selectedTask, getTaskComplexity, getComplexityStats, getComplexityTrends, getComplexityInsights]);

  const getComplexityLevelColor = (level: string) => {
    switch (level) {
      case 'simple': return theme.colors.success;
      case 'moderate': return theme.colors.warning;
      case 'complex': return theme.colors.error;
      case 'very-complex': return theme.colors.error;
  ...
  const getComplexityLevelText = (level: string) => {
    switch (level) {
      case 'simple': return 'Basit';
      case 'moderate': return 'Orta';
      case 'complex': return 'Karmaşık';
      case 'very-complex': return 'Çok Karmaşık';
          ...
          <View style={{
            ...
            <View style={{
              ...
              <View style={{
                ...
                height: 12,
                borderRadius: 6,
                backgroundColor: getComplexityLevelColor(selectedTaskComplexity.level),
                marginRight: 10,
              }} />
              <Text style={{
                fontSize: 16,
                ...
                {getComplexityLevelIcon(selectedTaskComplexity.level)} {getComplexityLevelText(selectedTaskComplexity.level)}
              </Text>
              <Text style={{
                fontSize: 16,
            ...
            <View style={{ marginBottom: 15 }}>
              ...
              {Object.entries(selectedTaskComplexity.factors).map(([key, value]) => (
                <View key={key} style={{
                  flexDirection: 'row',
                  justifyContent: 'space-between',
                  alignItems: 'center',
            ...
            {selectedTaskComplexity.recommendations.length > 0 && (
              <View>
                ...
                {selectedTaskComplexity.recommendations.map((recommendation, index) => (
          ...
          {complexityLevels.map((level) => (
            <TouchableOpacity
              ...
              style={{
                ...
                paddingVertical: 8,
                borderRadius: 20,
                backgroundColor: selectedLevel === level.key ? theme.colors.primary : theme.colors.surface,
                marginRight: 10,
                borderWidth: 1,
                borderColor: selectedLevel === level.key ? theme.colors.primary : theme.colors.border,
              }}
            >
              <Text style={{
                fontSize: 14,
                color: selectedLevel === level.key ? theme.colors.background : theme.colors.text,
        ...
        {complexityStats && (
          <View style={{ padding: 20 }}>
            ...
            <Card style={{ marginBottom: 15 }}>
              ...
              {Object.entries(complexityStats.complexityDistribution).map(([level, count]) => (
                <View key={level} style={{
                  ...
                  <Text style={{
                    ...
                    {getComplexityLevelIcon(level)} {getComplexityLevelText(level)}
                  </Text>
                  <Text style={{
                    fontSize: 14,
                    fontWeight: 'bold',
            ...
            {complexityStats.mostComplexTasks.length > 0 && (
              <Card style={{ marginBottom: 15 }}>
                ...
                {complexityStats.mostComplexTasks.slice(0, 3).map((item: any, index: number) => (
          ...
          <View style={{ padding: 20 }}>
            ...
            <Card style={{ marginBottom: 15 }}>
              ...
              <Text style={{
                ...
                Karmaşıklık trendi: {complexityTrends.trend === 'increasing' ? '📈 Artıyor' : 
                                   complexityTrends.trend === 'decreasing' ? '📉 Azalıyor' : '📊 Stabil'}
              </Text>
            ...
            {complexityInsights.recommendations.length > 0 && (
              <Card>
                ...
                {complexityInsights.recommendations.map((recommendation: string, index: number) => (
                  <Text key={index} style={{
                    fontSize: 14,
                    color: theme.colors.textSecondary,
                    marginLeft: 10,
