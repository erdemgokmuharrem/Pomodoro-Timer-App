...
export class AnalyticsEngine {
  ...
  calculateUserMetrics(): UserMetrics {
    const completedSessions = this.sessions.filter(s => s.isCompleted && !s.isBreak);
  ...
  generateInsights(): AnalyticsInsight[] {
    ...
    if (metrics.performance.focusScore < 60) {
      insights.push({
        ...
        description: 'Odaklanma skorunuz düşük. Kesintileri azaltmaya odaklanın.',
        confidence: 0.8,
        actionable: true,
    ...
    if (metrics.patterns.consistencyScore < 0.6) {
      insights.push({
        ...
        description: 'Çalışma programınızda tutarsızlık var. Düzenli saatlerde çalışmayı deneyin.',
        confidence: 0.7,
        actionable: true,
    ...
    if (metrics.performance.energyLevel < 50) {
      insights.push({
        ...
        description: 'Enerji seviyeniz düşük. Daha fazla mola almayı ve dinlenmeyi deneyin.',
        confidence: 0.8,
        actionable: true,
  ...
  private calculatePreferredBreakTimes(): number[] {
    ...
    const average = breakDurations.reduce((sum, d) => sum + d, 0) / breakDurations.length;
  ...
  private calculateWorkStreak(): number {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const hasWork = this.sessions.some(s => {
        ...
        return sessionDate.toDateString() === date.toDateString() && s.isCompleted;
      });
  ...
  private calculateConsistencyScore(): number {
    ...
    const averageActivity = dailyActivity.reduce((sum, count) => sum + count, 0) / 7;
    const variance = dailyActivity.reduce((sum, count) => sum + Math.pow(count - averageActivity, 2), 0) / 7;
  ...
  private calculateProductivityTrend(): number[] {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayPomodoros = this.sessions.filter(s => 
  ...
  private calculateFocusTrend(): number[] {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      if (daySessions.length === 0) {
        ...
        const totalInterruptions = daySessions.reduce((sum, s) => sum + s.interruptions, 0);
        const focusScore = Math.max(0, 100 - (totalInterruptions / daySessions.length) * 10);
  ...
  private calculateMoodTrend(): number[] {
    ...
    return this.calculateProductivityTrend().map(pomodoros => Math.min(100, pomodoros * 25));
  }
  ...
  private generateRecommendations(): string[] {
    ...
    if (metrics.performance.focusScore < 70) {
      recommendations.push('Kesintileri azaltmak için çalışma ortamınızı düzenleyin');
    }
